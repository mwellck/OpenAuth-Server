	...
	if($request['content-type'] == "application/json") {
		...
		if(!empty($req))
			...
			if($req->clientToken == $clientToken) {
				...
				Core\Queries::execute('UPDATE ".$config['dbprefix']."users SET accessToken=:accessToken WHERE clientToken=:clientToken', ['accessToken' => $newAccessToken, 'clientToken' => $clientToken]);
